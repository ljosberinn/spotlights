...
---
--- They must be separate frames: outside a group `GetGroupHeaderType` returns no kind, so
--- `SecureGroupHeader_Update` configures with an empty table and every child hides itself
--- (`SecureGroupHeaders.lua:404-408`), and no attribute overrides that.
---
...
--- Dimming is how a stand-in is told apart from a live spotlight, and a spacer is dimmer still so it reads
--- as a hole. The options pane passes neither, showing the opacity setting as it is.
local GRID_DIM = 0.7
local BLANK_DIM = 0.25

--- Fraction of maximum health. Enough of the bar to read as a shield rather than an artefact.
...
---
--- Prefers the slot's *configured* name, so previewing a populated grid shows the people who will be in
--- it. An unassigned slot falls back to a fabricated label; a spacer is left blank to read as a hole.
---
--- Shared with the options frame's preview pane (`Options/PreviewPane.lua`), which renders the same
--- appearance block onto the same template. `dim` and `class` are the only things the two disagree about.
...
function Private.Preview.Fill(frame, index, slot, dim, class)
	...
	-- Texture before colour: SetStatusBarTexture resets the region's vertex colour to white, the same trap
	-- UpdateTexture documents on the real frames.
	...
	-- The same static-or-class choice the live frame makes. A preview never disconnects or dies, so that
	-- branch of the live path has no counterpart here.
...
function Private.Preview.Place(index, point, x, y, config, slot)
	-- Nothing is created until previewing is first switched on: `ApplyContainer` runs on every roster event
	-- and zone change, so building unconditionally would keep a grid's worth of frames, and the overlay
	-- they parent to, alive from login for a feature that may never be used.
	if not shown then
	...
	-- The *same* point and offsets the header uses against the container, and Mover.Sync keeps the two
	-- rectangles identical, so a preview lands exactly where its spotlight will.
	...
	-- Occupancy is the header's *child*, tested with `IsVisible`: the header is shown at creation and stays
	-- so forever, while the child is what the secure update shows and hides.
...
---
--- Routed through the layout pass rather than looping frames here, because `Fill` needs each cell's slot
--- and offsets, which `ApplyContainer` already has. A no-op when nothing is being previewed, so callers
--- need not ask whether it matters.
...
--- The cell whose preview is under the cursor, or nil. The drop target for a cell nobody is in yet, which
--- out of a group is every cell, so this is what makes the grid assignable before a group exists.
---
--- The index needs no compaction lookup: `Place` fills preview *i* from `slots[i]`. A hidden preview cannot
--- match, since `IsCursorOver` tests visibility, so occupied cells fall through to
--- `Private.SlotHeader.CellUnderCursor` rather than being claimed twice.
---@return integer? slot
...
function Private.Preview.SetShown(value)
	...
	-- The container's visibility is a secure state driver keyed on `[group]`, so showing previews outside a
	-- group means taking that condition over for the duration.
	Private.Container.SetPreviewing(value)

	if value then
		-- The full request, not just Layout: previews anchor to the overlay, which `Mover.Sync` aligns from
		-- the *Position* pass, after Layout has sized the container.
